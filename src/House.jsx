...
export function House(props) {
  const { nodes, materials } = useGLTF('/SOL.glb');
  ...
  const moveCameraBit = (bitcoin) => {
    camera.lookAt(bitcoin.current.position.x)
    console.log(bitcoin)
  }
  return (
    <group {...props} dispose={null}>
      <mesh ref={bitcoin}
        onClick={moveCameraBit}
        castShadow
        receiveShadow
      ...
      <group position={[0.386, -0.679, -0.338]} rotation={[-Math.PI / 2, 0, 0]} scale={0.001}>
        <mesh
          ...
          position={[-1349.429, -764.154, 306.615]}
        />
      </group>
      <group rotation={[-Math.PI / 2, 0, 0]} scale={0.188}>
        <group position={[0, 3, 6]} scale={0.843}>
          <group position={[3.348, -7.273, 8.839]} rotation={[0, 0, -1.508]} scale={0.607}>
            <mesh
              castShadow
              receiveShadow
      ...
      />
      <Html fullscreen>
        <div style={{ position: 'absolute', bottom: '20px', width: '100%', display: 'flex', justifyContent: 'center', pointerEvents: 'none' }}>
          <button
            onClick={moveCamera}
            style={{
              padding: '10px 20px',
